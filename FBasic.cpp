...
/*SETUP KINECT*/
HRESULT CKinectFusionBasics::Ksetup()
{
	HRESULT hr;
...
/// </summary>
void CKinectFusionBasics::Update(cv::Mat &m)
{
...
HRESULT CKinectFusionBasics::SetupUndistortion()
{
	...
	if (m_cameraParameters.principalPointX != 0)
	{
		...
		for (UINT rowID = 0; rowID < m_cDepthHeight; rowID++)
		{
			...
			if (FAILED(hr))
			{
				SetStatusMessage("Failed to initialize Kinect Coordinate Mapper.");
				return hr;
			}
		}
		...
		{
			SetStatusMessage("Failed to initialize Kinect Fusion depth image distortion Lookup Table.");
...
HRESULT CKinectFusionBasics::OnCoordinateMappingChanged()
{
	...
	{
		SetStatusMessage("Failed to initialize Kinect Fusion depth image distortion buffer.");
		return E_OUTOFMEMORY;
	}
...
HRESULT CKinectFusionBasics::InitializeKinectFusion()
{
	...
	if (FAILED(hr))
	{
		...
		{
			CHAR buf[MAX_PATH];
			sprintf_s(buf, ARRAYSIZE(buf), "Device %d not able to run Kinect Fusion, or error initializing.", m_deviceIndex);
			SetStatusMessage(buf);
		}
		else if (E_NUI_GPU_OUTOFMEMORY == hr)
		{
			CHAR buf[MAX_PATH];
			sprintf_s(buf, ARRAYSIZE(buf), "Device %d out of memory error initializing reconstruction - try a smaller reconstruction volume.", m_deviceIndex);
			SetStatusMessage(buf);
		}
		else if (NUI_FUSION_RECONSTRUCTION_PROCESSOR_TYPE_CPU != m_processorType)
		{
			CHAR buf[MAX_PATH];
			sprintf_s(buf, ARRAYSIZE(buf), "Failed to initialize Kinect Fusion reconstruction volume on device %d.", m_deviceIndex);
			SetStatusMessage(buf);
		}
		else
		{
	...
	SetStatusMessage("Click ¡®Reset Reconstruction' to clear!");
...
/// get mesh data
///</summary>
void CKinectFusionBasics::getMeshData(cv::Vec3f* &p, int &Vcount, int *&tri,cv::Vec3f* &n)
{
	HRESULT hr = S_OK;
	// Calculate Mesh!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!                                M  E S H
	INuiFusionMesh *pMesh = nullptr;
	hr = m_pVolume->CalculateMesh(1, &pMesh);
	if (!FAILED(hr))
	{
		...
		hr = pMesh->GetTriangleIndices(&triangles);
		hr = pMesh->GetNormals(&normals);
		p = (cv::Vec3f *)vertices;
		tri = (int*)triangles;
		n = (cv::Vec3f *)normals;
	}
...
/// </summary>
void CKinectFusionBasics::ProcessDepth(cv::Mat &m)
{
	if (m_bInitializeError)
	{
		return;
	...
	m = cv::Mat(m_cDepthHeight, m_cDepthWidth, CV_8UC4, pBuffer);
	...
	*/
	...
	if ((int)elapsed >= cTimeDisplayInterval)
	{
		...
		{
			CHAR str[MAX_PATH];
			sprintf_s(str, ARRAYSIZE(str), "Fps: %5.2f", fps);
...
void CKinectFusionBasics::SetStatusMessage(CHAR * szMessage)
{
	printf("%s\n", szMessage);
}
